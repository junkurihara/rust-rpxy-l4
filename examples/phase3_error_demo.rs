...
use rpxy_l4_lib::{ProxyError, NetworkError, ProtocolError, ConnectionError, ConfigurationError, ErrorContext};
...
fn main() {
    ...
    let io_result: Result<(), std::io::Error> = Err(
        std::io::Error::new(std::io::ErrorKind::PermissionDenied, "Permission denied")
    );
    
    let with_context = io_result.with_connection_context(
        "127.0.0.1:8080".parse().unwrap(),
        "192.168.1.1:80".parse().unwrap()
    );
    
    match with_context {
        Err(ProxyError::Connection(ConnectionError::Broken { source_addr, dest_addr, reason })) => {
            println!("   Context Added - Source: {}, Dest: {}, Reason: {}", 
