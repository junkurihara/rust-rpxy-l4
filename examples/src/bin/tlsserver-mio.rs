...
impl OpenConnection {
  fn new(socket: TcpStream, token: mio::Token, mode: ServerMode, tls_conn: rustls::ServerConnection) -> Self {
...
fn load_crls(filenames: impl Iterator<Item = impl AsRef<Path>>) -> Vec<CertificateRevocationListDer<'static>> {
  filenames
    .map(|filename| CertificateRevocationListDer::from_pem_file(filename).expect("cannot read CRL file"))
    .collect()
}
