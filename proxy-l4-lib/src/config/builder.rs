...
impl ConfigBuilder {
    ...
    pub fn with_tcp_target(mut self, targets: Vec<&str>) -> Result<Self, ConfigValidationError> {
        let target_addrs: Result<Vec<TargetAddr>, _> = targets
            .iter()
            .map(|s| s.parse::<TargetAddr>().map_err(|e| ConfigValidationError::TargetAddressError {
...
mod tests {
    ...
    fn test_config_builder_with_targets() {
        let config = ConfigBuilder::new()
            .with_listen_port(8080).unwrap()
            .with_tcp_target(vec!["192.168.1.1:80", "example.com:80"]).unwrap()
