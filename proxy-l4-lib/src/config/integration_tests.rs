...
#[cfg(test)]
mod tests {
    use crate::{ConfigBuilder, ProtocolConfigBuilder, ProtocolType, LoadBalance};

    #[test]
    fn test_config_builder_basic_usage() {
        let config = ConfigBuilder::new()
            .with_listen_port(8080).unwrap()
            .with_ipv6(true)
            .with_tcp_backlog(1024).unwrap()
            .with_tcp_max_connections(1000).unwrap()
            .with_udp_max_connections(2000).unwrap()
            .with_tcp_target(vec!["192.168.1.1:80", "192.168.1.2:80"]).unwrap()
            .with_tcp_load_balance(LoadBalance::SourceIp).unwrap()
            .build().unwrap();
    ...
    fn test_protocol_config_builder() {
        let tls_protocol = ProtocolConfigBuilder::new()
            ...
            .with_targets(vec!["192.168.1.1:443", "192.168.1.2:443"]).unwrap()
            .with_load_balance(LoadBalance::SourceIp).unwrap()
            .with_alpn(vec!["h2".to_string(), "http/1.1".to_string()]).unwrap()
            .with_server_names(vec!["example.com".to_string()]).unwrap()
            .build().unwrap();
    ...
    fn test_config_builder_with_protocol() {
        let tls_protocol = ProtocolConfigBuilder::new()
            .with_protocol(ProtocolType::Tls)
            .with_targets(vec!["192.168.1.1:443"]).unwrap()
            .with_alpn(vec!["h2".to_string()]).unwrap()
            .build().unwrap();
        
        let config = ConfigBuilder::new()
            .with_listen_port(443).unwrap()
            .with_protocol("https".to_string(), tls_protocol).unwrap()
            .build().unwrap();
    ...
    fn test_comprehensive_config_build() {
        ...
        let ssh_protocol = ProtocolConfigBuilder::new()
            ...
            .with_targets(vec!["10.0.0.1:22"]).unwrap()
            .build().unwrap();
        
        let wireguard_protocol = ProtocolConfigBuilder::new()
            .with_protocol(ProtocolType::Wireguard)
            .with_targets(vec!["10.0.0.1:51820"]).unwrap()
            .with_idle_lifetime(60).unwrap()
            .build().unwrap();
        
        let config = ConfigBuilder::new()
            .with_listen_port(8080).unwrap()
            .with_ipv6(false)
            .with_tcp_backlog(2048).unwrap()
            .with_tcp_target(vec!["192.168.1.1:80"]).unwrap()
            .with_udp_target(vec!["8.8.8.8:53"]).unwrap()
            .with_udp_idle_lifetime(30).unwrap()
            .with_protocol("ssh".to_string(), ssh_protocol).unwrap()
            .with_protocol("wireguard".to_string(), wireguard_protocol).unwrap()
            .build().unwrap();
