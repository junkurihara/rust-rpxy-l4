use crate::config::{EchProtocolConfig, validation::*};
...
impl TlsProtocolConfig {
  ...
  pub fn validate(&self, protocol_name: &str) -> Result<(), ConfigValidationError> {
    ProtocolValidator::validate_tls_config(protocol_name, self.alpn.as_deref(), self.server_names.as_deref())
  }
...
impl EchConfigBuilder {
  ...
  pub fn build(self) -> Result<EchProtocolConfig, ConfigValidationError> {
    ...
    EchProtocolConfig::try_new(&config_list, &self.private_keys, &self.private_server_names, &listen_port)
      .map_err(|e| ConfigValidationError::EchConfigurationError { reason: e.to_string() })
  }
}
...
mod tests {
  ...
  fn test_tls_validation_errors() {
    // Empty ALPN
    let result = TlsProtocolConfig::new().with_alpn(vec!["".to_string()]);
    assert!(result.is_err());

    // Empty server name
    let result = TlsProtocolConfig::new().with_server_names(vec!["".to_string()]);
    assert!(result.is_err());

    // Server name with space
    let result = TlsProtocolConfig::new().with_server_names(vec!["invalid domain.com".to_string()]);
    assert!(result.is_err());
  }
