...
impl From<ConfigValidationError> for ProxyBuildError {
    fn from(err: ConfigValidationError) -> Self {
        ProxyBuildError::BuildMultiplexersError(err.to_string())
    }
}
...
impl BasicConfigValidator {
    ...
    pub fn validate_connection_limits(
        tcp_max: Option<u32>,
        udp_max: Option<u32>,
    ) -> ValidationResult<()> {
    ...
    pub fn validate_dns_cache_ttl(
            ...
            (Some(min), Some(max)) => {
                if min > max {
                    return Err(ConfigValidationError::ConflictingConfiguration {
                        reason: format!(
                            "DNS cache min_ttl ({:?}) cannot be greater than max_ttl ({:?})",
                            min, max
                        ),
                    });
                }
        ...
        if let Some(max) = max_ttl {
            if max > Duration::from_secs(86400) { // 24 hours
                crate::trace::warn!(
...
impl TargetValidator {
    ...
    pub fn validate_load_balance_with_targets(
...
impl ProtocolValidator {
    ...
    pub fn validate_quic_config(
        ...
        if let Some(alpn_list) = alpn {
            ...
            if !has_quic_alpn {
                crate::trace::warn!(
                    "QUIC config '{}' doesn't specify h3 ALPN - ensure this is intended",
                    name
                );
            }
        }
...
mod tests {
    ...
    fn test_validate_tls_config() {
        ...
        assert!(ProtocolValidator::validate_tls_config(
            "test",
            alpn.as_deref(),
