...
use bytes::BytesMut;
use quic_tls::{probe_tls_handshake, TlsClientHelloBuffer, TlsProbeFailure};
...
impl ProtocolDetector<TcpProtocol> for SshDetector {
    fn detect<'a>(&'a self, buffer: &'a mut BytesMut) -> Pin<Box<dyn Future<Output = Result<ProbeResult<TcpProtocol>, ProxyError>> + Send + 'a>> {
        Box::pin(async move {
            if buffer.len() < 4 {
